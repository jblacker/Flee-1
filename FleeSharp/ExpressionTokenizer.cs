...
namespace Flee
{
    ...
    internal class ExpressionTokenizer : Tokenizer
    {
        ...
        {
            TokenPattern pattern = new TokenPattern((int)ExpressionConstants.ADD, "ADD", TokenPattern.PatternType.String, "+");
            ...
            customPattern.Initialize((int)ExpressionConstants.ARGUMENT_SEPARATOR, "ARGUMENT_SEPARATOR", TokenPattern.PatternType.String, ",", this.myContext);
            ...
            this.AddPattern(pattern);
            this.AddPattern(new TokenPattern((int)ExpressionConstants.WHITESPACE, "WHITESPACE", TokenPattern.PatternType.Regexp, "\\s+")
            {
                Ignore = true
            });
            pattern = new TokenPattern((int) ExpressionConstants.INTEGER, "INTEGER", TokenPattern.PatternType.Regexp, "\\d+(u|l|ul|lu|f|m)?");
            this.AddPattern(pattern);
            customPattern = new RealPattern();
            customPattern.Initialize((int)ExpressionConstants.REAL, "REAL", TokenPattern.PatternType.Regexp, "\\d{0}\\{1}\\d+([e][+-]\\d{{1,3}})?(d|f|m)?", this.myContext);
            this.AddPattern(customPattern);
