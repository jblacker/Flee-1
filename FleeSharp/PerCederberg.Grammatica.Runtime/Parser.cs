...
namespace Flee.PerCederberg.Grammatica.Runtime
{
    ...
    internal abstract class Parser
	{
		...
		public Tokenizer Tokenizer => this.tokenizer;

	    public Analyzer Analyzer => this.analyzer;
		...
		public virtual void AddPattern(ProductionPattern pattern)
		{
			...
			if (flag2)
			{
				throw new ParserCreationException(ParserCreationException.ErrorType.INVALID_PRODUCTION, pattern.Name, 
                    ...
                    + (double)pattern.Id + Conversions.ToDouble(") has already been added")));
			}
		...
		private void CheckElement(string name, ProductionPatternElement elem)
		{
			...
			if (flag)
			{
				throw new ParserCreationException(ParserCreationException.ErrorType.INVALID_PRODUCTION, name,
                    ...
                    (double)elem.Id + Conversions.ToDouble(") is referenced")));
			}
		}
		...
		internal Token NextToken()
		{
			...
			throw new ParseException(ParseException.ErrorType.UnexpectedEof, null, this.tokenizer.GetCurrentLine(), this.tokenizer.GetCurrentColumn());
		}
