...
namespace Ciloci.Flee
{
	internal class InElement : ExpressionElement
	{
		...
		private void ResolveForCollectionSearch()
		{
			...
			if (flag)
			{
				base.ThrowCompileException("SearchArgIsNotKnownCollectionType", CompileExceptionReason.TypeMismatch, new object[]
				{
			...
			if (flag2)
			{
				base.ThrowCompileException("OperandNotConvertibleToCollectionType", CompileExceptionReason.TypeMismatch, new object[]
				{
		...
		private void EmitListIn(FleeILGenerator ilg, IServiceProvider services, BranchManager bm)
		{
			...
			try
			{
				...
				while (enumerator.MoveNext())
				{
					...
					InElement.EmitBranchToTrueTerminal(ilg, trueTerminal, bm);
				}
			}
			finally
			{
