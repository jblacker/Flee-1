	...
	public sealed class NamespaceImport : ImportBase, ICollection<ImportBase>
	{
		...
		protected override void AddMembers(string memberName, MemberTypes memberType, ICollection<MemberInfo> dest)
		{
			try
			{
				...
				{
					ImportBase import = enumerator.Current;
					ImportBase.AddImportMembers(import, memberName, memberType, dest);
				}
			}
			finally
			{
		...
		{
			return string.Equals(this.MyNamespace, name, base.Context.Options.MemberStringComparison);
		}

		protected override bool EqualsInternal(ImportBase import)
		{
			NamespaceImport otherSameType = import as NamespaceImport;
			return otherSameType != null && this.MyNamespace.Equals(otherSameType.MyNamespace, base.Context.Options.MemberStringComparison);
		}

		public void Add(ImportBase item)
		{
			Utility.AssertNotNull(item, "item");
			bool flag = base.Context != null;
			if (flag)
			{
				item.SetContext(base.Context);
			}
