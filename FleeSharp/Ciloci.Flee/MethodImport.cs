...
namespace Ciloci.Flee
{
	public sealed class MethodImport : ImportBase
	{
		...
		{
			base.Context.AssertTypeIsAccessible(this.MyMethod.ReflectedType);
		}

		protected override void AddMembers(string memberName, MemberTypes memberType, ICollection<MemberInfo> dest)
		{
			bool flag = string.Equals(memberName, this.MyMethod.Name, base.Context.Options.MemberStringComparison) && (memberType & MemberTypes.Method) > (MemberTypes)0;
			if (flag)
			{
		...
		{
			return string.Equals(this.MyMethod.Name, name, base.Context.Options.MemberStringComparison);
		}
