	...
	internal abstract class MemberElement : ExpressionElement
	{
		...
		protected virtual void Validate()
		{
			...
			if (!flag)
			{
				...
				if (flag2)
				{
					base.ThrowCompileException("StaticMemberCannotBeAccessedWithInstanceReference", CompileExceptionReason.TypeMismatch, new object[]
					{
						this.MyName
					});
				}
				else
				{
					...
					if (flag3)
					{
						base.ThrowCompileException("ReferenceToNonSharedMemberRequiresObjectReference", CompileExceptionReason.TypeMismatch, new object[]
						{
							this.MyName
						});
					}
		...
		{
			MemberElement.EmitMethodCall(this.ResultType, this.NextRequiresAddress, mi, ilg);
		}
		...
		protected static void EmitMethodCall(Type resultType, bool nextRequiresAddress, MethodInfo mi, FleeILGenerator ilg)
		{
			...
			if (flag)
			{
				MemberElement.EmitReferenceTypeMethodCall(mi, ilg);
			}
			else
			{
				MemberElement.EmitValueTypeMethodCall(mi, ilg);
			}
			bool flag2 = resultType.IsValueType & nextRequiresAddress;
			if (flag2)
			{
				MemberElement.EmitValueTypeLoadAddress(ilg, resultType);
			}
		}
		...
		private static void EmitValueTypeMethodCall(MethodInfo mi, FleeILGenerator ilg)
		{
			...
			else
			{
				...
				if (flag)
				{
					bool flag2 = MemberElement.IsGetTypeMethod(mi);
					if (flag2)
					{
		...
		protected void EmitLoadOwner(FleeILGenerator ilg)
		{
			...
			if (!flag)
			{
				...
				if (requiresAddress)
				{
					MemberElement.EmitValueTypeLoadAddress(ilg, ownerType);
				}
			}
		}
		...
		{
			bool flag = MemberElement.IsMemberPublic(member);
			bool accessAllowed;
			if (flag)
			{
		...
		public bool IsMemberAccessible(MemberInfo member)
		{
			...
			if (flag)
			{
				IsMemberAccessible = MemberElement.IsOwnerMemberAccessible(member, this.MyOptions);
			}
			else
			{
				IsMemberAccessible = MemberElement.IsMemberPublic(member);
			}
