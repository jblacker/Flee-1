...
namespace Ciloci.Flee
{
	...
	{
		private readonly Type myLeftType;

		private readonly Type myRightType;
		...
		public override MethodBase SelectMethod(BindingFlags bindingAttr, MethodBase[] match, Type[] types, ParameterModifier[] modifiers)
		{
			checked
			{
				...
				for (int i = 0; i < match.Length; i++)
				{
					...
					bool leftValid = ImplicitConverter.EmitImplicitConvert(this.myLeftType, parameters[0].ParameterType, null);
					bool rightValid = ImplicitConverter.EmitImplicitConvert(this.myRightType, parameters[1].ParameterType, null);
					bool flag = leftValid & rightValid;
					if (flag)
					{
						selectMethod = mi;
						return selectMethod;
