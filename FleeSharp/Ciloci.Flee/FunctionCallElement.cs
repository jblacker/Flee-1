...
namespace Ciloci.Flee
{
	internal class FunctionCallElement : MemberElement
	{
		...
		protected override bool RequiresAddress
		{
			get
			{
				return !MemberElement.IsGetTypeMethod(this.Method);
			}
		}
		...
		protected override void ResolveInternal()
		{
			...
			if (flag)
			{
				MemberInfo[] arr = base.GetMembers(MemberTypes.Method);
		...
		private void ThrowFunctionNotFoundException(MemberElement previous)
		{
			...
			if (flag)
			{
				base.ThrowCompileException("UndefinedFunction", CompileExceptionReason.UndefinedName, new object[]
				{
					this.MyName,
					this.MyArguments
				});
			...
			{
				base.ThrowCompileException("UndefinedFunctionOnType", CompileExceptionReason.UndefinedName, new object[]
				{
					this.MyName,
					this.MyArguments,
		...
		private void ThrowNoAccessibleMethodsException(MemberElement previous)
		{
			...
			if (flag)
			{
				base.ThrowCompileException("NoAccessibleMatches", CompileExceptionReason.AccessDenied, new object[]
				{
					this.MyName,
					this.MyArguments
				});
			...
			{
				base.ThrowCompileException("NoAccessibleMatchesOnType", CompileExceptionReason.AccessDenied, new object[]
				{
					this.MyName,
					this.MyArguments,
					previous.TargetType.Name
		...
		{
			base.ThrowCompileException("AmbiguousCallOfFunction", CompileExceptionReason.AmbiguousMatch, new object[]
			{
				this.MyName,
				this.MyArguments
			});
		...
		protected override void Validate()
		{
			...
			if (!flag)
			{
				...
				if (flag2)
				{
					base.ThrowCompileException("FunctionHasNoReturnValue", CompileExceptionReason.FunctionHasNoReturnValue, new object[]
					{
						this.MyName
					});
				}
		...
		public override void Emit(FleeILGenerator ilg, IServiceProvider services)
		{
			...
			else
			{
				...
				this.EmitFunctionCall(base.NextRequiresAddress, ilg, services);
			}
		}
		...
		private void EmitOnDemandFunction(ExpressionElement[] elements, FleeILGenerator ilg, IServiceProvider services)
		{
			...
			FunctionCallElement.EmitElementArrayLoad(elements, typeof(object), ilg, services);
		...
		private void EmitParamArrayArguments(ParameterInfo[] parameters, ExpressionElement[] elements, FleeILGenerator ilg, IServiceProvider services)
		{
			...
			FunctionCallElement.EmitElementArrayLoad(paramArrayElements, this.MyTargetMethodInfo.paramArrayElementType, ilg, services);
		}
