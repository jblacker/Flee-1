...
namespace Ciloci.Flee
{
	internal class FleeExpressionAnalyzer : ExpressionAnalyzer
	{
		...
		{
			node.AddValues(base.GetChildValues(node));
			return node;
		}
		...
		{
			IList childValues = base.GetChildValues(node);
		...
		{
			node.AddValue(RuntimeHelpers.GetObjectValue(base.GetChildAt(node, 0).Values[0]));
		}
